...
def set(**kwargs):
    ...
    if kwargs["digitos"]:
        res = dotenv.set_key(env_file, "DIGITOS", kwargs["digitos"])
        click.echo("%s=%s" % (res[1], res[2]))
        return 0
    # Altera o nome da barra lateral
    if kwargs["lateral"]:
        res = dotenv.set_key(env_file, "LATERAL", kwargs["lateral"].upper())
        click.echo("%s=%s" % (res[1], res[2]))
        return 0
    # Altera o nome da barra de alta
    if kwargs["alta"]:
        res = dotenv.set_key(env_file, "ALTA", kwargs["alta"].upper())
        click.echo("%s=%s" % (res[1], res[2]))
        return 0
    # Altera o nome da barra de baixa
    if kwargs["baixa"]:
        res = dotenv.set_key(env_file, "BAIXA", kwargs["baixa"].upper())
        click.echo("%s=%s" % (res[1], res[2]))
        return 0
    # Altera a abreviatura da barra de rompimento de alta
    if kwargs["rompimento_alta"]:
        res = dotenv.set_key(env_file, "ROMPIMENTO_ALTA", kwargs["rompimento_alta"].upper())
        click.echo("%s=%s" % (res[1], res[2]))
        return 0
    # Altera a abreviatura da barra de rompimento de baixa
    if kwargs["rompimento_baixa"]:
        res = dotenv.set_key(env_file, "ROMPIMENTO_BAIXA", kwargs["rompimento_baixa"].upper())
        click.echo("%s=%s" % (res[1], res[2]))
        return 0
    # Altera o percentual do corpo da barra doji
    if kwargs["percentual_doji"]:
        res = dotenv.set_key(env_file, "PERCENTUAL_DOJI", kwargs["percentual_doji"])
        click.echo("%s=%s" % (res[1], res[2]))
        return 0
    ...
    if kwargs["mt5_pasta"]:
        res = dotenv.set_key(env_file, "MT5_PASTA", kwargs["mt5_pasta"])
        click.echo("%s=%s" % (res[1], res[2]))
        return 0
    # Lista as variáveis disponíveis
    vars = dotenv.dotenv_values(env_file)
