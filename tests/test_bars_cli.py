...
def test_exibe_a_view_completo_doji():
    res = run.invoke(
        mt,
        ["bars", "--symbol", "abev3", "--view", "f", "--period", "d1", "--count", "1"],
    )
...
def test_exibe_a_view_completo_de_rompimento_de_baixa():
    res = run.invoke(
        mt,
        ["bars", "--symbol", "bbdc4", "--view", "f", "--period", "d1", "--count", "1"],
    )
...
def test_exibe_a_view_minimo():
    res = run.invoke(
        mt,
        ["bars", "--symbol", "abev3", "--period", "d1", "--count", "1", "--view", "ch"],
    )
...
def test_exibe_a_view_de_fechamentos():
    res = run.invoke(
        mt,
        ["bars", "--symbol", "abev3", "--period", "d1", "--count", "1", "--view", "c"],
    )
...
def test_exibe_a_view_de_maximas():
    res = run.invoke(
        mt,
        ["bars", "--symbol", "abev3", "--period", "d1", "--count", "1", "--view", "h"],
    )
...
def test_exibe_a_view_de_minimas():
    res = run.invoke(
        mt,
        ["bars", "--symbol", "abev3", "--period", "d1", "--count", "1", "--view", "l"],
    )
...
def test_exibe_a_view_de_volume():
    res = run.invoke(
        mt,
        ["bars", "--symbol", "abev3", "--period", "d1", "--count", "1", "--view", "v"],
    )
...
def test_exibe_a_view_de_ranges():
    res = run.invoke(
        mt,
        ["bars", "--symbol", "abev3", "--period", "d1", "--count", "1", "--view", "r"],
    )
...
def test_exibe_a_view_de_variacoes_percentuais():
    res = run.invoke(
        mt,
        ["bars", "--symbol", "bbdc4", "--period", "d1", "--count", "1", "--view", "va"],
    )
...
def test_exibe_view_minima_com_data():
    res = run.invoke(
        mt, ["bars", "--symbol", "bbdc4", "--view", "ch", "--show-date", "--count", "1"]
    )
...
def test_exibe_view_completa_com_data_ativada():
    res = run.invoke(
        mt, ["bars", "--symbol", "bbdc4", "--view", "f", "--count", "1", "--show-date"]
    )
    assert (
        res.output
...
def test_exibe_view_ranges_com_data_ativada():
    res = run.invoke(
        mt, ["bars", "--symbol", "bbdc4", "--view", "r", "--count", "1", "--show-date"]
    )
...
def test_exibe_view_da_variacao_percentual_com_data():
    res = run.invoke(
        mt, ["bars", "--symbol", "bbdc4", "--view", "va", "--count", "1", "--show-date"]
    )
    assert res.output == "0.69% -0.95% -0.78% 2025-02-27\n"


def test_exibe_view_volume_com_data():
    res = run.invoke(
        mt, ["bars", "--symbol", "bbdc4", "--view", "v", "--count", "1", "--show-date"]
    )
...
def test_exibe_view_aberturas_com_data():
    res = run.invoke(
        mt, ["bars", "--symbol", "bbdc4", "--view", "o", "--count", "1", "--show-date"]
    )
...
def test_exibe_view_maximas_com_data():
    res = run.invoke(
        mt, ["bars", "--symbol", "bbdc4", "--view", "h", "--count", "1", "--show-date"]
    )
...
def test_exibe_view_minimas_com_data():
    res = run.invoke(
        mt, ["bars", "--symbol", "bbdc4", "--view", "l", "--count", "1", "--show-date"]
    )
...
def test_exibe_view_fechamentos_com_data():
    res = run.invoke(
        mt, ["bars", "--symbol", "bbdc4", "--view", "c", "--count", "1", "--show-date"]
    )
...
def test_exibe_view_no_m1():
    res = run.invoke(
        mt,
        ["bars", "--symbol", "wdoj25", "--view", "f", "--period", "m1", "--count", "1"],
    )
    assert (
...
def test_exibe_view_no_m2():
    res = run.invoke(
        mt,
        ["bars", "--symbol", "wdoj25", "--view", "f", "--period", "m2", "--count", "1"],
    )
...
def test_exibe_view_no_m3():
    res = run.invoke(
        mt,
        ["bars", "--symbol", "wdoj25", "--view", "f", "--period", "m3", "--count", "1"],
    )
    assert (
        res.output
...
def test_exibe_view_no_m5():
    res = run.invoke(
        mt,
        ["bars", "--symbol", "wdoj25", "--view", "f", "--period", "m5", "--count", "1"],
    )
...
def test_exibe_view_no_m6():
    res = run.invoke(
        mt,
        ["bars", "--symbol", "wdoj25", "--view", "f", "--period", "m6", "--count", "1"],
    )
...
def test_exibe_view_no_h1():
    res = run.invoke(
        mt,
        ["bars", "--symbol", "wdoj25", "--view", "f", "--period", "h1", "--count", "1"],
    )
    assert res.output == "IB V VERMELHO80   5774.00 5769.00 5769.50M5771.50 R5.00\n"
...
def test_exibe_view_no_h2():
    res = run.invoke(
        mt,
        ["bars", "--symbol", "wdoj25", "--view", "f", "--period", "h2", "--count", "1"],
    )
    assert res.output == "IB V VERMELHO80   5774.00 5769.00 5769.50M5771.50 R5.00\n"
...
def test_exibe_view_no_h3():
    res = run.invoke(
        mt,
        ["bars", "--symbol", "wdoj25", "--view", "f", "--period", "h3", "--count", "1"],
    )
    assert res.output == "IB V VERMELHO80   5774.00 5769.00 5769.50M5771.50 R5.00\n"
...
def test_exibe_view_no_h4():
    res = run.invoke(
        mt,
        ["bars", "--symbol", "wdoj25", "--view", "f", "--period", "h4", "--count", "1"],
    )
...
def test_exibe_view_ohlc():
    res = run.invoke(
        mt,
        ["bars", "--symbol", "bbdc4", "--view", "oh", "--period", "d1", "--count", "1"],
    )
